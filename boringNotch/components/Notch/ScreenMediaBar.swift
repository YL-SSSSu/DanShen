...
final class ScreenMediaBarController: NSObject, NSWindowDelegate {
    ...
    private var isSuppressedByAuxiliaryWindow = false
    ...
    func setSuppressedByAuxiliaryWindow(_ isSuppressed: Bool) {
        guard isSuppressedByAuxiliaryWindow != isSuppressed else { return }
        isSuppressedByAuxiliaryWindow = isSuppressed
        updateVisibility()
    }

    private func updateVisibility() {
        updateWindowLevel()
        let shouldShow = !isSuppressedByAuxiliaryWindow
            && Defaults[.screenMediaBarEnabled]
    ...
    private func show() {
        ...
        installContentIfNeeded(on: panel)
    ...
    private func installContentIfNeeded(on panel: NSPanel) {
