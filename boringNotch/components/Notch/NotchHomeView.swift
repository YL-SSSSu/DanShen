...
private struct HomeWeatherCard: View {
    ...
    var body: some View {
        HomeWidgetCard(
            title: "天气",
            subtitle: subtitle,
            headerActionImage: "arrow.clockwise",
            headerActionHelp: "刷新天气",
            headerActionDisabled: manager.isRefreshing,
            headerAction: {
                Task {
            ...
            if let snapshot = manager.snapshot {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if manager.locationAuthorizationStatus == .denied || manager.locationAuthorizationStatus == .restricted {
                        actionButton("定位设置", systemImage: "location.slash") {
...
private struct HomeWidgetCard<Content: View>: View {
    let title: String
    let subtitle: String
    let headerActionImage: String?
    let headerActionHelp: String?
    let headerActionDisabled: Bool
    ...
    init(
        title: String,
        subtitle: String,
        headerActionImage: String? = nil,
        headerActionHelp: String? = nil,
        headerActionDisabled: Bool = false,
        headerAction: (() -> Void)? = nil,
        @ViewBuilder content: () -> Content
    ) {
        self.title = title
        self.subtitle = subtitle
        self.headerActionImage = headerActionImage
        self.headerActionHelp = headerActionHelp
        self.headerActionDisabled = headerActionDisabled
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                ...
                if let headerActionImage, let headerAction {
                    Button(action: headerAction) {
                        Image(systemName: headerActionImage)
                    ...
                    .help(headerActionHelp ?? title)
                    .accessibilityLabel(headerActionHelp ?? title)
                }
            }
