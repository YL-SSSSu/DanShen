...
enum AIProviderCredentialStore {
    // A local ad-hoc build has no provisioning profile, so it cannot use the
    // iOS-style data-protection keychain. Keep this item in the macOS keychain
    // and use a fresh namespace so legacy ACLs never trigger an access prompt.
    ...
    private static func saveAPIKeySynchronously(_ rawValue: String) throws {
        ...
        try writeKeychainValue(value, service: service, account: account)
        Defaults[.aiServiceAPIKey] = ""
    }

    private static func writeKeychainValue(
        _ value: String,
        service: String,
        account: String
    ) throws {
    ...
    private static func readKeychainValue(
        service: String = AIProviderCredentialStore.service,
        account: String = AIProviderCredentialStore.account
    ) throws -> String {
    ...
    #if DEBUG
    static func runRuntimeSelfTest() -> [String: Bool] {
        let testService = "\(service).runtime-self-test"
        let testAccount = "ephemeral-test-value"
        let marker = UUID().uuidString

        do {
            try writeKeychainValue(marker, service: testService, account: testAccount)
            let storedValue = try readKeychainValue(service: testService, account: testAccount)
            try writeKeychainValue("", service: testService, account: testAccount)
            let deletedValue = try readKeychainValue(service: testService, account: testAccount)
            return [
                "write": true,
                "read": storedValue == marker,
                "delete": deletedValue.isEmpty,
            ]
        } catch {
            try? writeKeychainValue("", service: testService, account: testAccount)
...
final class AIChatManager: ObservableObject {
    ...
    func refreshCredentialState() {
        ...
        credentialRefreshTask = Task { [weak self] in
            ...
            self?.hasConfiguredAPIKey = !apiKey.isEmpty
        }
    }
...
final class WeatherManager: ObservableObject {
    ...
    private init() {
        ...
        automaticRefreshTask = Task { [weak self] in
            await self?.refreshWeather(force: false)
