...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @MainActor
    var hasOpenAssistantNotch: Bool {
        guard coordinator.currentView == .assistant else { return false }
        if Defaults[.showOnAllDisplays] {
            return viewModels.values.contains { $0.notchState == .open }
        }
    ...
    func prepareForAuxiliaryWindowPresentation() {
        ...
        let activeViewModels = [vm] + Array(viewModels.values)

        for viewModel in activeViewModels {
            viewModel.suppressHoverAutoOpen(for: 2.0)
            if viewModel.notchState == .open {
                viewModel.closeForAuxiliaryWindow()
            }
        }
    }
    ...
    private func handleDragEntersNotchRegion(onScreen screen: NSScreen) {
        guard !AppWindowPresentationCoordinator.shared.hasPresentedWindows else { return }
        guard let uuid = screen.displayUUID else { return }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--credential-store-self-test") {
            let result = AIProviderCredentialStore.runRuntimeSelfTest()
